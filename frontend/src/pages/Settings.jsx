...
export default function Settings() {
  ...
  async function handleTestConnection() {
    ...
    try {
      const res = await testConnection({ api_key: form.api_key, secret_key: form.secret_key, testnet: form.testnet });
      setConnResult(res);
  ...
  async function handleTestTelegram() {
    setTestingTg(true); setTgResult(null);
    try {
    ...
    } catch (e) { setTgResult({ success: false, message: e.message }); }
    setTestingTg(false);
  }
  ...
  async function handleTestEmail() {
    ...
    } catch (e) { setEmailResult({ success: false, message: e.message }); }
    ...
    <div className="page settings-page">
      ...
      <form onSubmit={handleSave} className="settings-form">
        ...
        <div className="settings-card">
          ...
          {form.testnet
            ? <div className="settings-note">Testnet ON — real funds NOT used. <a className="link" href="https://testnet.binance.vision" target="_blank" rel="noreferrer">testnet.binance.vision</a></div>
            ...
            <div className="form-group" style={{ flex:2 }}>
              ...
              <div className="input-with-icon">
                <input id="api_key" type={showKey?"text":"password"} name="api_key"
                  placeholder="Paste your Binance API key" value={form.api_key}
                  onChange={handleChange} className="form-input" autoComplete="off"/>
            ...
            <div className="form-group" style={{ flex:2 }}>
              ...
              <div className="input-with-icon">
                <input id="secret_key" type={showSecret?"text":"password"} name="secret_key"
                  placeholder="Paste your Binance Secret key" value={form.secret_key}
                  onChange={handleChange} className="form-input" autoComplete="off"/>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
            ...
            {connResult && (
              <div className={`conn-result ${connResult.connected?"success":"error"}`}>
                {connResult.connected ? `✅ Connected — $${Number(connResult.usdt_balance||0).toFixed(2)} USDT` : `❌ ${connResult.error}`}
              </div>
            )}
          </div>
        </div>
        ...
        {/* ── Advanced Risk ── */}
        <div className="settings-card">
          <div className="settings-section-title"><TrendingDown size={15}/> Advanced Risk & AI</div>
          <div className="form-row">
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <small>Use Fear & Greed + news to filter signals</small>
            </div>
            <div className="form-group">
              ...
              <small>Analyze 15m, 1h, 4h together for stronger signals</small>
            </div>
          </div>
        </div>
        ...
        {/* ── AI Indicators ── */}
        <div className="settings-card">
          <div className="settings-section-title"><Brain size={15}/> AI Indicator Settings</div>
          <div className="form-row">
            <div className="form-group">
        ...
        <div className="settings-card">
          ...
          {telegramOk && <div className="key-status saved">✅ Telegram active</div>}
          <div className="form-row">
            <div className="form-group" style={{ flex:2 }}>
              <label>Bot Token</label>
              <div className="input-with-icon">
                <input type={showTgToken?"text":"password"} name="telegram_bot_token"
                  placeholder="1234567890:ABCdef..." value={form.telegram_bot_token}
                  onChange={handleChange} className="form-input" autoComplete="off"/>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            {tgResult && <div className={`conn-result ${tgResult.success?"success":"error"}`}>{tgResult.success?"✅ ":"❌ "}{tgResult.message}</div>}
          </div>
        </div>
        ...
        <div className="settings-card">
          ...
          <div className="settings-note">
            Normal password kaam nahi — <b>App Password</b> chahiye:&nbsp;
            <a className="link" href="https://myaccount.google.com/apppasswords" target="_blank" rel="noreferrer">myaccount.google.com/apppasswords</a>
          </div>
          {emailOk && <div className="key-status saved">✅ Gmail active</div>}
          <div className="form-row">
            <div className="form-group" style={{ flex:2 }}>
              <label>Your Gmail</label>
              <input type="email" name="email_sender" placeholder="yourbot@gmail.com"
                value={form.email_sender} onChange={handleChange} className="form-input" autoComplete="off"/>
            </div>
            <div className="form-group" style={{ flex:2 }}>
              <label>App Password</label>
              <div className="input-with-icon">
                <input type={showEmailPwd?"text":"password"} name="email_app_password"
                  placeholder="xxxx xxxx xxxx xxxx" value={form.email_app_password}
                  onChange={handleChange} className="form-input" autoComplete="off"/>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            {emailResult && <div className={`conn-result ${emailResult.success?"success":"error"}`}>{emailResult.success?"✅ ":"❌ "}{emailResult.message}</div>}
          </div>
        </div>
